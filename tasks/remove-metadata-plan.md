...
| account keys (validator stash `AccountId32`) | `ParaSessionInfo.AccountKeys` typed storage | manual `AccountKeys` key (`Identity` hasher) + positional — `session_info` holds `ValidatorId`, not stash accounts |
...
- [x] 6. Validator groups → `ParachainHost_validator_groups`. Metadata-free `decode_validator_groups` (`api/decode.rs`, unit-tested): the runtime returns `(Vec<Vec<ValidatorIndex>>, GroupRotationInfo)` — codec `decode_all`s the tuple, keeps the groups (each `ValidatorIndex` / rotation field a transparent `u32`), drops the rotation info, fails loud on trailing bytes. Moved the inlined `ParaScheduler.ValidatorGroups` dynamic-storage read out of the executor arm into `ApiClient::get_backing_groups`, which stays primary; under `--shadow-decode-without-metadata` it also fetches via `state_call("ParachainHost_validator_groups")` and `shadow::compare`s both as `Vec<Vec<u32>>` (order preserved — groups are indexed). **Shadow verified live** against Polkadot: clean over 16 heads (`write_backing_groups` → `get_backing_groups` runs every head), zero mismatches. Switching to primary and deleting the dynamic path happens with task 17.
- [x] 7. Session index → `session_index_for_child`; account keys → manual `ParaSessionInfo.AccountKeys` storage key (NOT `session_info` — see below). Metadata-free `decode_session_index` (bare `u32`) and `decode_account_keys` (`Vec<AccountId32>`) + `para_session_account_keys_key` (`api/decode.rs`, unit-tested). The `AccountKeys` map uses the `Identity` hasher, so the key is `twox_128("ParaSessionInfo") ++ twox_128("AccountKeys") ++ encoded_index` (no per-key hash). Shadows live inside `ApiClient`: `get_session_index` keeps the `Session.CurrentIndex` storage read primary and compares it against `state_call("ParachainHost_session_index_for_child")`; `get_session_account_keys` keeps the typed `AccountKeys` read primary and compares against `state_get_storage` of the manual key (only at a concrete hash — at latest the two reads could race to different blocks). **Correction to the table below:** `session_info` holds sr25519 `ValidatorId` public keys, not the stash `AccountId32`s the tools use (whois address matching, dispute attribution) — so account keys stay a storage read via a hand-built key, same data as today. **Shadow verified live** against Polkadot: clean over 15 heads including a session change (13396), zero mismatches — `Identity` hasher confirmed correct. `get_session_index_now` (whois one-shot, no hash) left unshadowed; it adopts the runtime call when switched to primary. Boundary caveat: `session_index_for_child` tracks `CurrentIndex` in the same rotation, so they match; a rare 1-block skew at a session boundary would surface as a shadow mismatch. Switch to primary and delete typed paths with task 17.
