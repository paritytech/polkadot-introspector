...
impl ChainHeadSubscription {
	...
	async fn run_per_node(
		...
		let sub_best = match executor.get_best_head_subscription(&url).await {
			Ok(v) => v.map_ok(|v| ChainSubscriptionEvent::NewBestHead(v.0.hash())),
			Err(e) => {
				error!("Subscription to {} failed: {:?}", url, e);
				std::process::exit(1)
			},
		};
		let sub_finalized = match executor.get_finalized_head_subscription(&url).await {
			Ok(v) => v.map_ok(|v| ChainSubscriptionEvent::NewFinalizedBlock(v.0.hash())),
			Err(e) => {
				error!("Subscription to {} failed: {:?}", url, e);
				std::process::exit(1)
			},
		};
		let mut sub = select(sub_best, sub_finalized);
